...
/** Small utility commands: identity, diagnostics, and lightweight helpers. */
...
/**
 * Pull the media out of a replied-to view-once message. Replies to the user
 * with the reason and returns undefined if it can't.
 */
async function extractViewOnce(m: Message): Promise<Revealed | undefined> {
  // Content is unwrapped by Message.quoted, but unwrap defensively too.
  const content = unwrapContent(m.quoted?.raw);
  if (!content) {
    await m.reply('Reply to a view-once photo or video.');
    return undefined;
  }
...
/** .vv — reveal privately to your DM, leaving only a reaction in the chat. */
command(
  { pattern: 'vv', desc: 'Reveal a view-once — privately to your DM', usage: '<reply>', category: 'tools', fromMe: true },
  async (m) => {
    const media = await extractViewOnce(m);
    if (!media) return;
    const target = vaultJid(m);
    try {
      if (media.isImage) await m.client.sendMessage(target, { image: media.buffer, caption: media.caption });
      else await m.client.sendMessage(target, { video: media.buffer, caption: media.caption });
      await m.react('📩');
      ...
      // Fallback so you still get the media.
      if (media.isImage) await m.sendImage(media.buffer, media.caption);
      else await m.sendVideo(media.buffer, media.caption);
    }
  },
);

/** .vvhere — reveal in the current chat instead of your DM. */
command(
  { pattern: 'vvhere', desc: 'Reveal a view-once in this chat', usage: '<reply>', category: 'tools', fromMe: true },
  async (m) => {
    const media = await extractViewOnce(m);
    if (!media) return;
    if (media.isImage) await m.sendImage(media.buffer, media.caption);
