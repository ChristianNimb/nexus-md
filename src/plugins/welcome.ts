...
/**
 * Toggle and customize the welcome/goodbye messages fired by
 * core/events.ts when members join or leave. Templates support the tokens
 * @user, @group and @count.
 */

function onOff(arg: string | undefined): boolean | undefined {
  const v = arg?.trim().toLowerCase();
  if (v === 'on' || v === 'enable') return true;
  if (v === 'off' || v === 'disable') return false;
  return undefined;
}

command(
  { pattern: 'welcome ?(.*)', desc: 'Toggle/set welcome message', usage: 'on|off | <template>', category: 'group', groupOnly: true, adminOnly: true },
  async (m, match) => {
...
command(
  { pattern: 'goodbye ?(.*)', desc: 'Toggle/set goodbye message', usage: 'on|off | <template>', category: 'group', groupOnly: true, adminOnly: true },
  async (m, match) => {
    const arg = match?.[1]?.trim();
    const toggle = onOff(arg);
    if (toggle !== undefined) {
