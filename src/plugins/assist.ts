...
/**
 * Little problem-solvers people reach for constantly.
 *
 *   .tldr   (reply to a long message)   → a tight summary
 *   .fix    (reply to a message)        → fixed grammar / cleaner wording
 *   .split 120 4  (+15%)                → who owes what
 */

/** Text from the command arg, or the message it's replying to. */
function subject(m: Message, arg: string): string {
...
command({ pattern: 'tldr ?(.*)', desc: 'Summarise a long message', usage: '<text | reply>', category: 'tools' }, async (m, match) => {
  ...
  if (text.length < 40) return m.reply('📝 Reply to a long message with *.tldr* (or paste the text after it).');
  await m.react('📝');
...
command({ pattern: 'fix ?(.*)', desc: 'Fix grammar / reword a message', usage: '<text | reply>', category: 'tools' }, async (m, match) => {
  const text = subject(m, match?.[1] ?? '');
  if (!text) return m.reply('✍️ Reply to a message with *.fix* (or type *.fix <your text>*) and I\'ll clean up the grammar.');
  await m.react('✍️');
