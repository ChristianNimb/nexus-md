...
import { logYtdlpReady } from './core/downloaders/index.js';
...
import { startStatusReports } from './core/status-report.js';
async function main() {
    if (!acquireLock()) {
        process.exit(1);
    }
    ...
    if (config.nexus.key && /(?:localhost|127\.0\.0\.1|host\.docker\.internal)/i.test(config.nexus.url)) {
        logger.warn(`AI endpoint is local (${config.nexus.url}, model ${config.nexus.model}). ` +
            'This needs Ollama running on the same host. On a cloud deploy set ' +
            'NEXUS_API_URL to a hosted provider (e.g. Groq) and NEXUS_MODEL to one it serves.');
    }
