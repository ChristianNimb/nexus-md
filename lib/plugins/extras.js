...
command({ pattern: 'catchup ?(.*)', desc: 'Summarise the recent messages you missed', usage: '[how many]', category: 'ai' }, async (m, match) => {
    const n = Math.min(120, Math.max(5, Number((match?.[1] ?? '').trim()) || 40));
    const msgs = recentMessages(m.chat, n);
    if (msgs.length < 3)
        return m.reply('🙂 Not enough recent messages to summarise yet. Check back after a bit of chatting.');
    const transcript = msgs.map((x) => `${x.name}: ${x.text}`).join('\n');
    await m.react('📝');
    const summary = await quickGen(`Summarise this WhatsApp ${m.isGroup ? 'group ' : ''}conversation for someone who was away. Give the key points, decisions, questions asked, and anything that needs a reply. As short, clear bullet points. Be concise.\n\n---\n${transcript}`);
