...
command({ pattern: 'catchup ?(.*)', desc: 'Summarise the recent activity you missed', usage: '[how many]', category: 'ai' }, async (m, match) => {
    ...
    if (events.length < 3)
        return m.reply(card({ icon: '📝', state: 'info', title: 'Nothing to catch up on', body: ['Not enough recent activity yet.'] }));
    ...
    await m.reply(summary
        ? card({ icon: '📝', state: 'info', title: 'Catch-up', body: [summary] })
        : card({ state: 'fail', title: 'Couldn’t summarise', hint: 'try again in a sec' }));
});
