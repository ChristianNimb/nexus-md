...
function denyIfNotOwner(m) {
    if (m.isRealOwner)
        return false;
    void m.reply('🔒 Contacts are private. Only my owner can access them.');
    return true;
}
command({ pattern: 'contacts(?: (reset|clear|wipe))?', fromMe: true, desc: 'Show the contacts Nexus knows (owner DM only)', category: 'tools' }, async (m, match) => {
    if (denyIfNotOwner(m))
        return;
...
command({ pattern: 'addcontact (.+)', fromMe: true, desc: 'Save a contact by name + number', usage: '<name> <number>', category: 'tools' }, async (m, match) => {
    if (denyIfNotOwner(m))
        return;
    const raw = (match?.[1] ?? '').trim();
    const numMatch = raw.match(/\+?\d[\d\s-]{6,}\d/);
    if (!numMatch)
        return m.reply('Usage: .addcontact <name> <number>. E.g. .addcontact Khalil 8613800138000');
    const number = numMatch[0];
    const name = raw.replace(number, '').replace(/\|/g, '').trim();
    if (!name)
        return m.reply('Give a name too: .addcontact Khalil 8613800138000');
    const res = await saveContactByNumber(m.client, name, number);
    if (!res.ok)
        return m.reply('That number looks too short. Include the country code, e.g. 8613800138000.');
...
command({ pattern: 'delcontact (.+)', fromMe: true, desc: 'Remove a saved contact', usage: '<name | number>', category: 'tools' }, async (m, match) => {
    if (denyIfNotOwner(m))
        return;
    const q = (match?.[1] ?? '').trim();
    if (!q)
        return m.reply('Usage: .delcontact <name | number>');
    const n = forgetContact(q);
    await m.reply(n ? `🗑️ Removed ${n} contact(s) matching ${q}.` : `No saved contact matched ${q}.`);
});
command({ pattern: 'whois (.+)', fromMe: true, desc: 'Look up a contact by name', usage: '<name>', category: 'tools', hidden: true }, async (m, match) => {
    if (denyIfNotOwner(m))
        return;
    const matches = findContacts((match?.[1] ?? '').trim());
    if (!matches.length)
        return m.reply('No match.');
